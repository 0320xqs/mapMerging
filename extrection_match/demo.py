...
def ByFlann(img1, img2, kp1, kp2, des1, des2, flag="ORB"):
    ...
    # matches = flann.match(des1, des2)#不用knn
...
def RANSAC(img1, img2, kp1, kp2, matches):
    ...
    if len(good) > MIN_MATCH_COUNT:
        ...
        # M: 3x3 变换矩阵.
        M, mask = cv2.findHomography(src_pts, dst_pts, cv2.RANSAC, 5.0) #计算两组对应点之间的单应性矩阵,mask是掩码矩阵，它标识了哪些匹配是内点（inliers）而哪些是外点（outliers）
    ...
    cv2.imshow("now", img3)
    # cv2.imwrite("缩放情况_SURF.jpg", img33)
    # cv2.imwrite("scale_SURF.jpg", img3)
    cv2.waitKey(0)


if __name__ == '__main__':
    img1 = cv2.imread("asset/image/part02-2.jpg")
    img2 = cv2.imread("asset/image/part02-2_small.jpg")
    kp1, des1 = SURF(img1)
    kp2, des2 = SURF(img2)
