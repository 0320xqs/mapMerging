...
def detect_and_highlight_right_angles(input_image_path, output_image_path):
    ...
    canny_edges = cv2.resize(edges_colored, (600, 400))
    cv2.imshow('Canny Edges:GREEN', canny_edges)
    # cv2.waitKey(0)

    # 3. 对提取的边缘执行RANSAC直线段检测并用蓝色突出显示
    lines = cv2.HoughLinesP(edges, 1, np.pi / 90, threshold=12, minLineLength=12, maxLineGap=0)
    ...
    if lines is not None:
        ...
        hough_lines = cv2.resize(output_image, (600, 400))
        cv2.imshow('RANSAC Lines:BLUE', hough_lines)
        # cv2.waitKey(0)
        ...
        if right_angle_detected:
            ...
            right_angle = cv2.resize(output_final, (600, 400))
            other_2 = cv2.resize(other, (600, 400))
            cv2.imshow('Final Result', right_angle)
            cv2.imshow('other:', other_2)
            cv2.waitKey(0)
        else:
            print("未检测到直角墙线。")
    else:
...
def main():
    input_image_path = 'map/right-angle-detect.pgm'  # 输入图像的路径
    output_image_path = 'map_out/output_image.png'  # 输出图像的路径
